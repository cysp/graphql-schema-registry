...
import { createGraphRouteSchema } from "./lib/fastify/openapi/generated/operations/create-graph.ts";
import { createSubgraphRouteSchema } from "./lib/fastify/openapi/generated/operations/create-subgraph.ts";
import { getSubgraphRouteSchema } from "./lib/fastify/openapi/generated/operations/get-subgraph.ts";
import { listSubgraphsRouteSchema } from "./lib/fastify/openapi/generated/operations/list-subgraphs.ts";
import { updateSubgraphRouteSchema } from "./lib/fastify/openapi/generated/operations/update-subgraph.ts";
...
await test("subgraph routes integration with postgres", async (t) => {
  ...
  await t.test("supports full subgraph CRUD flow", async () => {
    ...
    try {
      ...
      const createdGraph = createGraphRouteSchema.response[201].parse(
        parseJson(createGraphResponse),
      );
      ...
      assert.deepStrictEqual(
        listSubgraphsRouteSchema.response[200].parse(parseJson(emptyListResponse)),
        [],
      );
      ...
      const createdSubgraph = createSubgraphRouteSchema.response[201].parse(
        parseJson(createSubgraphResponse),
      );
      ...
      assert.deepStrictEqual(
        getSubgraphRouteSchema.response[200].parse(parseJson(getSubgraphResponse)),
        createdSubgraph,
      );
      ...
      const updatedSubgraph = updateSubgraphRouteSchema.response[200].parse(
        parseJson(updateSubgraphResponse),
      );
      ...
      assert.deepStrictEqual(
        listSubgraphsRouteSchema.response[200].parse(parseJson(deletedListResponse)),
        [],
      );
